...
function modifyURLParameters(url, params) {
  ...
  for(const key in params){
    if(key == url1){
      url1.searchParams.set(key, params[key])
    }
    else {
      url1.searchParams.set(key, params[key])
    }
...
function checkURLParameters(url, params) {
  ...
  for(const elem of params){
    result[elem] = url1.searchParams.has(elem)
  }
  return result
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function manipulateQuery(url, options) {
  ...
  if(options.has('delete')){
    for(const value of options.get('delete')){
      url1.searchParams.delete(value)
    }
  }
  return url1.href
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function sortUrlParams(url) {
  ...
  const newObj = new URL(urlObj.origin + urlObj.pathname);
